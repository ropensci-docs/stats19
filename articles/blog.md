...
works, and provide ideas for future work building on the experience. Now
is a good time to report on the package: version
[`0.2.0`](https://cran.r-project.org/package=stats19) has just been
release on CRAN, which contains a few improvements, some of which are
used in this blog post (`ask = FALSE` in
[`get_stats19()`](https://docs.ropensci.org/stats19/reference/get_stats19.md),
for example, which makes it even quicker to get data with this package).
...
The large and complex STATS19 data from the UK’s Department for
...
for example (this page provides an excellent introduction to STATS19).
...
(note the capital letters). There is another document called
[STATS20](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/995423/stats20-2011.pdf),
which is the guidance to STATS19 officers filling in the STATS19 form.

An important point is that the dataset omits crashes in which nobody was
hurt. The Department for Transport (DfT) also names the dataset [STATS19
on the main web page that links to open access road crash
data](https://www.data.gov.uk/dataset/cb7ae6f0-4be6-4935-9277-47e5ce24a11f/road-accidents-safety-data).
...
statistics](https://www.who.int/data/gho/data/themes/road-safety), as
...
> people in 2022, 3 deaths per 100,000) and worldwide (1,250,000 people
...
it, let alone be able to analyze the data contained within the files and
convert the integers they contain into meaningful data.
...
certain years (e.g. 2016, 2022) but not for all of 1979 - 2022 or 2018
now. The largest chunk contains data from 1979.

#### Note

As of February 2021 we do not run any of the code in this vignette due
to CRAN check issues. See
<https://ropensci.org/blog/2019/02/26/stats19/> for the final version of
this vignette.
...
With those introductions out of the way, let’s see what the package can
...
any year between 1979 and 2022.
...
road crashes recorded by the police in 2022 across Great Britain. We can
explore the `crashes_2022` object a little more:
...
You can also get the raw data (if you really want!) to see how much more
useful the data is after it has been cleaned and labelled by the
`stats19` package, compared with the data provided by government:
...
Let’s now read in casualties that took place in West Yorkshire (using
...
Then we used the **dplyr** function
...
which pedestrians were hurt, from the `crashes_types`, using `leaflet`
package:
...
- Comparing these datasets with interventions such as [20 mph
  zones](https://pubmed.ncbi.nlm.nih.gov/20007666/) and links with
  street morphology (**sarkar_street_2018?**).
- The creation of more general software for accessing and working with
  road crash data worldwide.
- Making the data even more available by provide the data as part of an
  interactive web application, a technique successfully used in the
  Propensity to Cycle Tool (PCT) project hosted at
  [www.pct.bike/](https://www.pct.bike/) (this would likely take further
  times/resources beyond what we can provide in our spare time!).

For now, however, we want to take the opportunity to celebrate the
release of `stats19` 🎉, thank rOpenSci for a great review process 🙏
and let you know: the package and data are now out there, and are ready
to be used 🚀.
