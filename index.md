...
By allowing public access to properly labelled road crash data,
**stats19** aims to make road safety research more reproducible and
accessible.
...
[`get_stats19()`](https://docs.ropensci.org/stats19/reference/get_stats19.md)
requires `year` and `type` parameters, mirroring the provision of
STATS19 data files, which are categorised by year (from 1979 onward) and
type (with separate tables for crashes, casualties and vehicles, as
outlined below). The following command, for example, gets crash data
from 2023 (**note**: we follow the “crash not accident” campaign of
...
If there are multiple matches, you will be asked to choose from a range
of options. Providing just the year, for example, will result in the
following options:
...
![](https://ars.els-cdn.com/content/image/1-s2.0-S136984781500039X-gr9.jpg)
...
![](reference/figures/README-crash-date-plot-1.png)
...
![](reference/figures/README-crash-time-plot-1.png)
